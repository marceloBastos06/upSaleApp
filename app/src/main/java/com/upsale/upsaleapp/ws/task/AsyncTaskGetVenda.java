 ...
 */
public class AsyncTaskGetVenda extends Task<Object, Void, List<Venda>> {

    @Override
    protected List<Venda> doInBackground(Object... params) {
        try {
            Http http = (Http) params[0];
            String json = http.solicitar();
            return GSON.fromJson(json, new TypeToken<List<Venda>>(){}.getType());
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
